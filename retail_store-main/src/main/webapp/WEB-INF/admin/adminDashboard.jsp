  ...
  <aside class="w-64 bg-black/70 backdrop-blur-lg border-r border-white/10 min-h-screen px-6 py-8">
    ...
    <nav class="space-y-4 text-lg">
      ...
      <a href="<%= request.getContextPath() %>/admin/users"
         class="flex items-center text-white hover:text-cyan-400">
        <i class="fas fa-users mr-3"></i> Users
      </a>

      <a href="<%= request.getContextPath() %>/admin/categories"
         class="flex items-center text-white hover:text-yellow-300">
        <i class="fas fa-th-list mr-3"></i> Categories
      </a>

      <a href="<%= request.getContextPath() %>/admin/products"
         class="flex items-center text-white hover:text-purple-300">
        <i class="fas fa-box mr-3"></i> Products
      </a>

      <a href="<%= request.getContextPath() %>/admin/orders"
         class="flex items-center text-white hover:text-red-300">
        <i class="fas fa-shopping-cart mr-3"></i> Orders
      </a>

      <!-- ✅ CORRIGÉ : remplacé <c:url value='/'/> par request.getContextPath() -->
      <a href="<%= request.getContextPath() %>/index.jsp"
         class="flex items-center text-white hover:text-green-300">
        <i class="fas fa-home mr-3"></i> Home
      </a>

      <!-- Bouton Déconnexion -->
      <a href="<%= request.getContextPath() %>/LogoutServlet"
         class="flex items-center text-white hover:text-orange-300 mt-8">
